...
class leo_ar_follower(object):
    ...
    def call_trigger_function_service(self):
        # Trigger function that change the ARTag position
        rospy.wait_for_service('goal_trigger_ar')  # Wait for the service to be available
        try:
            trigger_function = rospy.ServiceProxy('goal_trigger_ar', Trigger)
            call_service = TriggerRequest()
            response = trigger_function(call_service)  # Call the service
            print("response_status: ", response)
            return response.success  # Return the success status
    ...
    def update_cmd(self):
        ...
        # call the trigger once the robot will be in the desired distance
        if abs(self.distance_pid_error_1) < 0.001:
            self.call_trigger_function_service()
            rospy.sleep(0.2)
        else:
            pass
